...
#best_epochs = {'resnet':14, 'alexnet':14, 'vgg':13, 'squeezenet':13, 'densenet':9, 'inception':14}
...
def train_model(model_name, model, dataloaders, criterion, optimizer, num_epochs=25, is_inception=False):
    ...
    for epoch in range(num_epochs):
        line = 'Epoch {}/{}'.format(epoch, num_epochs - 1)
        #file.write(line+'\n')
        print(line)
        line = '-' * 10
        #file.write(line+'\n')
        ...
        for phase in ['train', 'val']:
            ...
            for i,(inputs, labels) in enumerate(dataloaders[phase],0):
                ...
                for i in range(len(preds)):
                    ...
                    a = torch.index_select(preds,0, index)
                    b = torch.index_select(labels.data,0, index)
                    if(a == b and a == torch.tensor([1])):
                        tp+=1
                    if(a != b and a == torch.tensor([1])):
                        fp+=1
            ...
            line = '{} Loss: {:.4f} Acc: {:.4f}'.format(phase, epoch_loss, epoch_acc)
            file.write(line+'\n')
            print(line)
            print(phase,"confusion matrix:",int(tp), int(fp), int(tn), int(fn))
    ...
    line = 'Training complete in {:.0f}m {:.0f}s'.format(time_elapsed // 60, time_elapsed % 60)
    file.write(line+'\n')
    print(line)
...
#for every model in ['resnet', 'alexnet', 'vgg', 'squeezenet', 'densenet', 'inception']
